...
function calculateBaseBid(
  ...
  const valueMultiplier =
    playerValue.tier === 'high' ? 1.3 : playerValue.tier === 'medium' ? 1.1 : 0.9;
  baseBid *= valueMultiplier;

  const competitionMultiplier =
    leagueContext.competitionLevel === 'high'
      ? 1.2
      : leagueContext.competitionLevel === 'medium'
        ? 1.1
        : 1.0;
...
function determineStrategy(
  ...
  return budgetRatio < 0.2 ? 'conservative' : budgetRatio < 0.4 ? 'balanced' : 'aggressive';
}
  ...
  return {
    calculateOptimalBid: (data: BidData): BidRecommendation => {
      ...
      const confidence = determineConfidence(
        data.biddingPattern.sampleSize,
        data.playerValue.confidence
      );
